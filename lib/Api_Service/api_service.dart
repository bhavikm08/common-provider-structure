...
class ApiService with CommonWidgets {
  Future<http.Response> request({
    ...
    if (!_networkStatusService.connectionValue) {
      commonToast(errorMessage: "No Internet Connection");
    ...
    try {
      ...
      if (method == Method.POST) {
        response =
            await http.post(uri, body: jsonEncode(reqBody), headers: headers);
      } else if (method == Method.DELETE) {
        response =
            await http.delete(uri, body: jsonEncode(reqBody), headers: headers);
